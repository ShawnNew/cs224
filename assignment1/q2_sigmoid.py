...
TEST_CASE = [   # Test list
	...
	{
		"func" : sigmoid_grad,
		"input_data" : sigmoid(np.array([[0, 0], [1,1]])),
		"expected_output" : np.array(
				[[ 0.25      ,  0.25      ],
	...
	{
		"func" : sigmoid,
		"input_data" : np.array(range(12)).reshape(3, 4),
		"expected_output" : np.array(
	...
	{
		"func" : sigmoid_grad,
		"input_data" : sigmoid(np.array(range(12)).reshape(3, 4)),
		"expected_output" : np.array(
	...
	{
		"func" : sigmoid,
		"input_data" : np.array(range(0, -12, -1)).reshape(3, 4),
		"expected_output" : np.array(
	...
	{
		"func" : sigmoid_grad,
		"input_data" : sigmoid(np.array(range(0, -12, -1)).reshape(3, 4)),
		"expected_output" : np.array(
			[[  2.50000000e-01,   1.96611933e-01,   1.04993585e-01,   4.51766597e-02],
       		[  1.76627062e-02,   6.64805667e-03,   2.46650929e-03,	  9.10221180e-04],
       		[  3.35237671e-04,   1.23379350e-04,   4.53958077e-05,    1.67011429e-05]])
...
def test_sigmoid():
    ...
    for i, test_case in enumerate(TEST_CASE):
    	test_function = test_case["func"]
    	test_data = test_case["input_data"]
    	test_output = test_function(test_data)
    	output = test_case["expected_output"]
    	assert np.allclose(output, test_output, rtol=1e-07, atol=1e-06)   #assert is to return the result of the test
