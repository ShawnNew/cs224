...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    f = sigmoid(np.dot(outputVectors[target], predicted))
    cost -= np.log(f)

    grad[target] += predicted * (f - 1.0)
    gradPred += outputVectors[target] * (f - 1.0)

    for k in xrange(K):
    	samp = indices[k + 1]    
    	f = sigmoid(np.dot(-outputVectors[samp], predicted))
    	cost -= np.log(f)
    	grad[samp] += predicted * (1.0 - f)
    	gradPred += outputVectors[samp] * (1.0 - f)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    ### YOUR CODE HERE
    indice_vc = tokens.get(currentWord)
    vc = inputVectors[indice_vc]
    for j in contextWords:
    	indice = tokens.get(j)
        cost_j, gradPred_j, grad_j = word2vecCostAndGradient(vc, indice, outputVectors, dataset)
        cost += cost_j
        gradIn[indice_vc] += gradPred_j
        gradOut += grad_j
    ### END YOUR CODE
