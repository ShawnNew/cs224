...
def softmax(x):
    ...
    ### YOUR CODE HERE
    max_x = tf.reduce_max(x, axis=1, keep_dims=True)  # normalization
    x -= max_x                                       # to do normalization, minus the maximum of each row
    exponential = tf.exp(x)                          # compute exponential
    rowSum = tf.reduce_sum(exponential, axis=1, keep_dims=True)
    out = exponential / rowSum
    ### END YOUR CODE
